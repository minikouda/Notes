...
\foreach \c in {0.25,0.5,1,2,3} {
  \addplot[domain=0:360,samples=200,thick]
    ({2*sqrt(\c)*cos(x)},{3*sqrt(\c)*sin(x)});
}

% Gradient arrows
\def\s{0.8}
\foreach \px/\py in {1/1, 2/0.5, -1/1.5, -2/-1} {
  \pgfmathsetmacro{\dx}{\s*(\px/2)}
  \pgfmathsetmacro{\dy}{\s*(2*\py/9)}
  \addplot[-{Stealth[length=2mm]},very thick]
    coordinates {(\px,\py) (\px+\dx,\py+\dy)};
}
...
\begin{axis}[
  ...
  ymin=-4,ymax=4,
  xlabel={$x_1$}, ylabel={$x_2$},
  title={$g(x_1,x_2)=x_1x_2$},
  grid=both,
]

\foreach \c in {0.5,1,2} {
  \addplot[domain=0.4:4,samples=200,thick] ({x},{\c/x});
  \addplot[domain=-4:-0.4,samples=200,thick] ({x},{\c/x});
}
...
% Gradient arrows
\def\t{0.4}
\foreach \px/\py in {1/1, 2/0.5, -1/2, -2/-1} {
  \pgfmathsetmacro{\dx}{\t*(\py)}
  \pgfmathsetmacro{\dy}{\t*(\px)}
  \addplot[-{Stealth[length=2mm]},very thick]
    coordinates {(\px,\py) (\px+\dx,\py+\dy)};
}
