...
\begin{enumerate}
    ...
    \[
    \implies \hat \theta = \dfrac{X_2 + 2 X_3}{2 n}
    \]
...
\subsection{Application of Likelihood Ratio Test} % (fold)
\label{sub:Application of Likelihood Ratio Test}

Back to \hyperref[sub:PT]{Peason's Test}

\begin{enumerate}
    \item $H_0: \theta \in \Theta_0 $ vs $H_1: \theta \in \Theta_1 = \Theta \setminus \Theta_0$.

    For example, $X_1,\dots,X_k \sim Multinomial (n,p_1,\dots,p_k)$, \\$H_0: p_1 = g_1(\theta),\dots,p_k = g_k(\theta)$ Note that $\sum\limits_{j=1}^{k} g_j(\theta) = 1$
    $H_1:$ not null.

    Without parameter constraints, MLE is $\hat p_j = \dfrac{Y_j}{n}$, where $Y_j$ is the observed count in category $j$.

    \item Independent Test for multiple Populations
%% table 
...
\[
H_0: \text{independent} \quad i.e. p_{ij} = p_{i\cdot} p_{\cdot j} ,\qquad H_a : \text{ not null}
\]

Test statistic:

Under $H_0$, MLE is $\hat p_{ij} = p_{i \cdot} p_{\cdot j} = \dfrac{Y_{i \cdot}}{n} \dfrac{Y_{\cdot j}}{n}$, where $Y_{i \cdot} = \sum\limits_{j=1}^{m} Y_{ij}$ and $Y_{\cdot j} = \sum\limits_{i=1}^{k} Y_{ij}$.
Under $H_a$, MLE is $\hat p_{ij} = \dfrac{Y_{ij}}{n}$.

Thus,
\[T = 2 \log \dfrac{L(\hat p_{ij})}{L(\hat p_{i \cdot} \hat p_{\cdot j})} = 2 \sum\limits_{i=1}^{k} \sum\limits_{j=1}^{m} Y_{ij} \log \dfrac{Y_{ij}}{n p_{i \cdot} p_{\cdot j}} \xrightarrow{D} \chi^2_{(k-1)(m-1)}\]


\hyperref[sub:Rejection Sampling]{Rejection Sampling}

\hyperref[sub:GeoBays]{Geometry Decision Theory}
