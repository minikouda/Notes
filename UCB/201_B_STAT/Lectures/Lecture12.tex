...
\textbf{Wald Test:} Consider testing $H_0 : \theta = \theta_0$ versus 
$H_1 : \theta \neq \theta_0$. Let $\hat{\theta}_n$ be an estimator such that
\[
\frac{\hat{\theta}_n - \theta_0}{\widehat{\mathrm{se}}(\hat{\theta}_n)}
\overset{D}{\to} N(0,1).
\]
The size $\alpha$ Wald test rejects $H_0$ when $T > z_{\alpha/2}$, where
\[
T = \left| \frac{\hat{\theta}_n - \theta_0}{\widehat{\mathrm{se}}(\hat{\theta}_n)} \right|.
\]

We can show that asymptotically, the Wald test has size $\alpha$, and that it is
obtained by inverting the approximate $1 - \alpha$ normal-based CI for $\theta$.
...
\begin{eb}
Suppose that $X \sim \text{Bin}(m, p_1)$ and $Y \sim \text{Bin}(n, p_2)$. Construct a size $\alpha$ Wald test for
...
We have
\[
\hat p_1 = \frac{X}{m}, \qquad \hat p_2 = \frac{Y}{n},
\]
so the MLE of $p_1 - p_2$ is
\[
\hat p_1 - \hat p_2 = \frac{X}{m} - \frac{Y}{n}.
\]

Since $X \sim \text{Bin}(m,p_1)$ and $Y \sim \text{Bin}(n,p_2)$ are independent,
\[
\mathrm{Var}(\hat p_1 - \hat p_2)
= \mathrm{Var}(\hat p_1) + \mathrm{Var}(\hat p_2)
= \frac{p_1(1-p_1)}{m} + \frac{p_2(1-p_2)}{n}.
\]

Under the null hypothesis $H_0: p_1 - p_2 = 0$, we have $p_1 = p_2 = p$.
We estimate $p$ using the pooled estimator
\[
\hat p = \frac{X+Y}{m+n}.
\]

Thus, the estimated standard error of $\hat p_1 - \hat p_2$ is
\[
\widehat{\mathrm{se}}(\hat p_1 - \hat p_2)
= \sqrt{\frac{\hat p(1-\hat p)}{m} + \frac{\hat p(1-\hat p)}{n}}.
\]

The Wald test statistic is
\[
W = \frac{(\hat p_1 - \hat p_2) - 0}
{\widehat{\mathrm{se}}(\hat p_1 - \hat p_2)}.
\]

Asymptotically, under $H_0$, $W \overset{D}{\to} N(0,1)$.
Therefore, the size $\alpha$ Wald test rejects $H_0$ if
\[
\left| W \right| > z_{\alpha/2},
\]
where $z_{\alpha/2}$ is the $(1-\alpha/2)$ quantile of the standard normal
distribution.
\end{solution}

Rmk. The $\hat{se}$ here is different from the one in \href{https://bcourses.berkeley.edu/courses/1548317/files/folder/Section%20Handouts?preview=92769638}{Section handouts 5}.
The corresponding link in \href{https://edstem.org/us/courses/84592/discussion/7466043}{ED}. Please correct me if I am wrong.
\end{eb}
...
The $\bar X - \theta_1 \sim N(0, \frac{1}{n})$, where $\theta_1$ is the true value of $\theta$.
\end{solution}
\end{eb}
